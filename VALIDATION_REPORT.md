...
- 岗位处理结果的“投递情况”下拉框已移除“演练未发送”，当前选项严格为：全部、未投递、已填充未发送、发送成功、处理失败、发送失败。
- 本次仅收敛 GUI 筛选选项；runner 的演练记录状态、原始结果保存、三字段“且”关系、岗位相关度排序、地点选项和运行期动态刷新均未改动。
- 新增精确选项元组回归测试，防止后续重新暴露“演练未发送”或意外改变其余选项顺序；全量测试为 `150 passed`。
- 已使用 Python 3.13、Nuitka 4.1.3、MSVC 14.5、LTO 和 onefile 重新构建两个正式 EXE；两个 PE 均为 64 位 GUI 子系统 2，文件版本与产品版本均为 `0.2.1.0`，编译报告确认筛选模块已进入正式主程序，临时加固 staging 已删除。
- `Boss求职助手.exe`：27,790,848 字节，SHA-256 `51754CFF38AC3FAF02D9A981FB8190FF1D625A7DE558F399A68E2FC3180E2008`；`Boss登录浏览器.exe`：14,177,280 字节，SHA-256 `DA17031AFDF735A20D36F6D4B74A7807444C2A1792C028ED689CE91C28073A2D`。
- 新主 EXE 使用隔离的无服务调试端口安全启动，窗口标题为 `Boss 求职助手控制台-Win v0.2.1` 且响应正常；未点击“开始”，随后通过窗口关闭流程干净退出，确认无本次进程残留。

## 0.2.0 岗位处理结果联合筛选

- “岗位处理结果”工具栏已在“暂停 / 停止 / 开始”同一行加入岗位、地点、投递情况与“筛选”按钮；岗位默认为空，地点与投递情况默认“全部”，两个下拉框均包含“全部”。地点选项由用户当前填写的目标城市去重生成，筛选控件不加入运行条件锁定列表，因此任务运行中仍可设置并点击生效。
- 三个字段按“且”关系取交集。地点支持“广州”匹配“广州·天河区”等细化地点，投递情况覆盖未投递、演练未发送、已填充未发送、发送成功、处理失败和发送失败。岗位名经过 NFKC 与大小写归一化后，以英文/数字词组和汉字单元计算相关度：完全同名优先、完整查询被岗位名包含次之，再按用户关键词覆盖率、岗位名覆盖率与最长连续命中排序；同相关度严格保留原岗位序号。
- GUI 维护独立的原始结果集合与筛选后的 Treeview：点击“筛选”只重建视图，不修改岗位序号、顶部累计进度、runner 统计、JSON 或 MySQL；运行中新增结果会按当前已生效条件动态加入正确相关度位置，不匹配结果仍保留在原始集合和累计统计中。筛选后优先停留第一行，避免 Tk 滚动范围尚未重算时误判为底部并被下一条记录拉走；用户滚到底部后仍按原逻辑恢复跟随。
- 回归覆盖完整岗位名多结果按原序号置顶、“AI应用开发工程师”对“AI开发工程师 / AI应用开发 / 工程师 / AI”的大部分到部分匹配、无关岗位排除、岗位/广州/未投递三条件交集、运行中非匹配结果隐藏及后续精确结果动态插入、暂停修改目标城市时已生效地点筛选保持一致，以及既有手动浏览和快速连续刷新路径；`python -m pytest tests -q` 为 `149 passed`，`compileall` 通过，项目 `.venv` `pip check` 为 `No broken requirements found`。
- 真实 Tk 布局烟测在 1560×920 窗口下确认筛选控件与三个运行按钮同排且没有重叠；目标城市“广州，深圳”生成 `全部 / 广州 / 深圳`，组合筛选后只显示广州、未投递且岗位相关的记录，顶部累计仍保持全部原始岗位数量。该烟测只使用内存示例记录，没有启动 runner 或执行 Boss 业务动作。
- 已使用 Python 3.13、Nuitka 4.1.3、MSVC 14.5、LTO 和 onefile 重新构建两个正式 EXE；两个 PE 均为 64 位 GUI 子系统 2，文件版本与产品版本均为 `0.2.0.0`。编译报告确认 `boss_assistant.gui.app` 与新增 `boss_assistant.gui.result_filter` 均为 `CompiledPythonModule`，临时加固 staging 已删除。
- `Boss求职助手.exe`：27,790,848 字节，SHA-256 `2C4BBAC1B2D516150C58EA0CAE2D867F62C7F5988BE7D68EB2D779F17D7FA850`；`Boss登录浏览器.exe`：14,177,280 字节，SHA-256 `F65A91D1153198B490A9A678E93A773E1E38827AB1192A9E9BDA6C4E43B429B6`。
- 新主 EXE 使用隔离的无服务调试端口安全启动，窗口标题为 `Boss 求职助手控制台-Win v0.2.0` 且响应正常；未点击“开始”，随后通过窗口关闭流程干净退出，确认无本次 `Boss求职助手` 进程残留。
