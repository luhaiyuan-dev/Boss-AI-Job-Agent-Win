...
- 支持暂停后修改运行条件、运行模式、审核方式和 MySQL 设置；继续时按界面新值执行，并同步更新断点、JSON 与 MySQL 运行记录。
- 结果表默认持续跟随最新记录；手动离开底部查看历史时暂停跟随，重新滚到底部后自动恢复。
- 岗位处理结果支持岗位关键词、目标城市和投递情况三项联合筛选；岗位全称精确命中优先，部分关键词按相关度排序，运行中新增结果也会按已生效条件动态显示。
...
| 结果筛选 | 岗位、地点、投递情况按“且”关系组合；地点选项来自目标城市；投递情况依次为全部、发送成功、已填充未发送、未投递、处理失败、发送失败、已置顶待处理、已发送简历、HR已拒绝，已忽略；其中“已发送简历”同时归并直接发送与回复后发送，但不包含“此前已发送”；岗位支持全称或部分关键词，设置后点击“筛选”生效 |

运行中可暂停并修改目标公司数、公司/岗位/城市、薪资、规模、分数、周末、经验等筛选条件，以及运行模式、审核方式和 MySQL 设置；点击“继续”后统一比较并应用界面当前值。目标公司数是本轮累计目标，不是追加数量：例如已经完成 5 家时从 10 改为 5，会在下一安全控制点直接正常结束；改为 7 则只继续完成剩余 2 家。多求职意向会从当前意向开始，依据已完成数量重新分配剩余配额。求职意向本身在本轮启动后保持锁定，避免切换页面时造成断点身份错位。
...
当前 `0.2.2` 基线、自动化流程修复与既有 Nuitka 构建/环境包校验结果见 [VALIDATION_REPORT.md](VALIDATION_REPORT.md)。
